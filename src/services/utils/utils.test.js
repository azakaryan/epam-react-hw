...
describe('Utils tests', () => {
  test('removeBlankAttributesFromObject with empty string', () => {
    expect(Utils.removeBlankAttributesFromObject({ aa: 12, bb: '' })).toEqual({
      aa: 12,
    });
  });

  test('removeBlankAttributesFromObject with null', () => {
    expect(Utils.removeBlankAttributesFromObject({ aa: 12, bb: null })).toEqual(
      { aa: 12 },
    );
  });
  ...
  test('removeBlankAttributesFromObject with undefined', () => {
    expect(
      Utils.removeBlankAttributesFromObject({ aa: 12, bb: undefined }),
    ).toEqual({ aa: 12 });
  });

  test('removeBlankAttributesFromObject with empty array required', () => {
    expect(
      Utils.removeBlankAttributesFromObject({ aa: 12, bb: [] }, true),
    ).toEqual({ aa: 12 });
  });
  ...
  test('removeBlankAttributesFromObject with 0', () => {
    expect(Utils.removeBlankAttributesFromObject({ aa: 12, bb: 0 })).toEqual({
      aa: 12,
      bb: 0,
    });
  });
  ...
  test('removeBlankAttributesFromObject with array as an input', () => {
    expect(
      Utils.removeBlankAttributesFromObject([{ aa: 12, bb: '' }]),
    ).not.toEqual([{ aa: 12 }]);
  });

  test('removeBlankAttributesFromObject with nested Object as an input', () => {
    expect(
      Utils.removeBlankAttributesFromObject({
        aa: 12,
        bb: { bb1: undefined, bb2: 67, bb3: '' },
      }),
    ).not.toEqual({ aa: 12, bb: { bb2: 67 } });
  });
